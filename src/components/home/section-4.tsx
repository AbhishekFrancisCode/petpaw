...
export const MealPortionSize = ({ isMealplanPage = false }: { isMealplanPage: boolean }) => {
  ...
  return (
    <div
      className={twMerge(
        !isMealplanPage && "bg-[#C4DEE1]",
        "flex flex-col min-h-[600px] pt-8 m-2 md:m-0"
      )}
    >
      {title && (
        <Title
      ...
      <div className="flex flex-wrap justify-center items-center min-h-[400px] px-4 pt-8 gap-8 lg:gap-24">
        {dogSizes.map((e: any) => (
          <div key={e.size} className="relative w-full sm:w-72 md:w-64 lg:w-80 max-h-[350px]">
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center p-6 gap-3 md:gap-6 mt-auto max-w-[85%] mx-auto">
        <WhatsAppButton />
